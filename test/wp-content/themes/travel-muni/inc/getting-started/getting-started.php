...
if ( ! function_exists( 'travel_muni_getting_started_menu' ) ) :
	/**
	 * Adding Getting Started Page in admin menu
	 */
	function travel_muni_getting_started_menu() {
		add_theme_page(
			__( 'Getting Started', 'travel-muni' ),
			__( 'Getting Started', 'travel-muni' ),
			'manage_options',
			'travel-muni-getting-started',
			'travel_muni_getting_started_page'
		);
	}
endif;
add_action( 'admin_menu', 'travel_muni_getting_started_menu' );

if ( ! function_exists( 'travel_muni_getting_started_admin_scripts' ) ) :
	/**
	 * Load Getting Started styles in the admin
	 */
	function travel_muni_getting_started_admin_scripts( $hook ) {
		// Load styles only on our page
		if ( 'appearance_page_travel-muni-getting-started' != $hook ) {
			return;
		}
		wp_enqueue_style( 'travel-muni-getting-started', get_template_directory_uri() . '/inc/getting-started/css/getting-started.css', false, TRAVEL_MUNI_THEME_VERSION );

		wp_enqueue_script( 'layzr', get_template_directory_uri() . '/js/layzr.min.js', array( 'jquery' ), '2.0.4', true );
		wp_enqueue_script( 'travel-muni-getting-started', get_template_directory_uri() . '/inc/getting-started/js/getting-started.js', array( 'jquery' ), TRAVEL_MUNI_THEME_VERSION, true );
		wp_localize_script( 'travel-muni-getting-started', 'travel_muni_getting_started', array( 'ajax_url' => admin_url( 'admin-ajax.php' ) ) );
		wp_enqueue_script( 'travel-muni-recommended-plugin-install', get_template_directory_uri() . '/inc/getting-started/js/recommended-plugin-install.js', array( 'jquery', 'plugin-install', 'wp-util', 'updates' ), TRAVEL_MUNI_THEME_VERSION, true );
		$localize = array(
			'ajaxUrl'              => admin_url( 'admin-ajax.php' ),
			'ActivatingText'       => __( 'Activating', 'travel-muni' ),
			'DeactivatingText'     => __( 'Deactivating', 'travel-muni' ),
			'PluginActivateText'   => __( 'Activate', 'travel-muni' ),
			'PluginDeactivateText' => __( 'Deactivate', 'travel-muni' ),
			'SettingsText'         => __( 'Settings', 'travel-muni' ),
		);
		wp_localize_script( 'travel-muni-recommended-plugin-install', 'gs_page', $localize );
	}
endif;
add_action( 'admin_enqueue_scripts', 'travel_muni_getting_started_admin_scripts' );

if ( ! function_exists( 'travel_muni_call_plugin_api' ) ) :
	/**
	 * Plugin API
	 **/
	function travel_muni_call_plugin_api( $plugin ) {
...
function required_plugin_activate() {
	...
	if ( ! current_user_can( 'install_plugins' ) || ! isset( $_POST['init'] ) || ! $_POST['init'] ) {
		wp_send_json_error(
			array(
				'success' => false,
				'message' => __( 'No plugin specified', 'travel-muni' ),
			)
		);
	}
	...
	wp_send_json_success(
		array(
			'success' => true,
			'message' => __( 'Plugin Successfully Activated', 'travel-muni' ),
		)
	);
...
function required_plugin_deactivate() {
	...
	wp_send_json_success(
		array(
			'success' => true,
			'message' => __( 'Plugin Successfully Deactivated', 'travel-muni' ),
		)
	);
...
if ( ! function_exists( 'travel_muni_check_for_icon' ) ) :
	/**
	 * Check For Icon
	 **/
	function travel_muni_check_for_icon( $arr ) {
		if ( ! empty( $arr['svg'] ) ) {
...
if ( ! function_exists( 'travel_muni_theme_club_list' ) ) :
	/**
	 * Ajax Callback for Theme Club List
	 */
	function travel_muni_theme_club_list() {
		...
		if ( $theme_array ) {
			...
			foreach ( $theme_array as $theme_list ) {
				...
				$theme_link    = isset( $theme_list['info']['link'] ) ? $theme_list['info']['link'] : '';
				?>
			<div class="wptravelengine">
				<div class="theme-image">
					<a class="theme-link" href="<?php echo esc_url( $theme_link ); ?>" target="_blank" rel="nofollow">
						<img data-layzr="<?php echo esc_url( $theme_image ); ?>" src="" alt="<?php echo esc_attr( $theme_title ); ?>">
					</a>
				</div>
				<h3><a href="<?php echo esc_url( $theme_link ); ?>"><?php echo esc_html( $theme_title ); ?></a></h3>
			...
			if ( ! is_wp_error( $themes_list ) && 200 === wp_remote_retrieve_response_code( $themes_list ) ) {
				...
				printf( __( '%1$sThis theme feed seems to be temporarily down. Please check back later, or visit our <a href="%2$s" target="_blank">Themes Club page on WP Travel Engine</a>.%3$s', 'travel-muni' ), '<p>', esc_url( $themes_link ), '</p>' );
			}
		}
...
add_action( 'wp_ajax_theme_club_from_rest', 'travel_muni_theme_club_list' );

if ( ! function_exists( 'travel_muni_getting_started_page' ) ) :
	/**
	 * Callback function for admin page.
	 */
	function travel_muni_getting_started_page() {
		?>
		<div class="wrap getting-started">
					<h2 class="notices"></h2>
					<div class="intro-wrap">
						<div class="intro">
							<h3><?php printf( __( 'Getting started with %1$s v%2$s', 'travel-muni' ), TRAVEL_MUNI_THEME_NAME, TRAVEL_MUNI_THEME_VERSION ); ?></h3>
							<h4><?php printf( __( 'You will find everything you need to get started with %1$s below.', 'travel-muni' ), TRAVEL_MUNI_THEME_NAME ); ?></h4>
						</div>
					</div><!-- .intro-wrap -->
					<div class="panels">
							...
							<li data-id="plugins" class="current">
								<a id="plugins" href="javascript:void(0);">
									<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 12 18">
										...
										<?php esc_html_e( 'Recommended Plugins', 'travel-muni' ); ?>
								</a>
							</li>
							<li data-id="help">
								<a id="help" href="javascript:void(0);">
									<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22 22">
										...
										<?php esc_html_e( 'Getting Started', 'travel-muni' ); ?>
								</a>
							</li>
							<li data-id="support">
								<a id="support" href="javascript:void(0);">
									<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
										...
										<?php esc_html_e( 'FAQ\'s &amp; Support', 'travel-muni' ); ?>
								</a>
							</li>
							<li data-id="themes" class="ajax">
								<a id="themeclub" href="javascript:void(0);">
									<svg xmlns="http://www.w3.org/2000/svg" width="24.5" height="21.5" viewBox="0 0 24.5 21.5">
										...
										<?php esc_html_e( 'Add-Ons', 'travel-muni' ); ?>
								</a>
							</li>
						</ul>
