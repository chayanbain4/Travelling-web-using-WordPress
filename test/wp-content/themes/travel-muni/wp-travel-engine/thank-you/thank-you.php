...
if ( is_null( $booking ) || 'booking' !== $booking->post_type ) {
	return __( 'Invalid booking or booking has been removed.', 'travel-muni' );
}

$date_format         = get_option( 'date_format' );
$wte_settings        = get_option( 'wp_travel_engine_settings' );
$extra_service_title = isset( $wte_settings['extra_service_title'] ) && ! empty( $wte_settings['extra_service_title'] ) ? $wte_settings['extra_service_title'] : __( 'Extra Services:', 'travel-muni' );
...
<div class="thank-you-container">
	<h3 class="trip-details"><?php esc_html_e( 'Booking Details:', 'travel-muni' ); ?></h3>
	<div class="detail-container">
		<div class="detail-item">
			<strong class="item-label"><?php esc_html_e( 'Booking ID :', 'travel-muni' ); ?></strong>
			<span class="value"><?php echo esc_html( $booking->ID ); ?></span>
		</div>
		<div class="detail-item" style="text-align:center;justify-content:center;">
			<strong style="font-size:18px;font-weight:normal"><?php echo esc_html( _n( 'Trip Details', 'Trips Details', count( $booking->order_trips ), 'travel-muni' ) ); ?></strong>
		</div>
		<?php
		...
		foreach ( $order_trips as $key => $item ) :
			...
			<div class="detail-item">
				<strong class="item-label"><?php esc_html_e( 'Trip ID:', 'travel-muni' ); ?></strong>
			...
			?>
			<div class="detail-item">
				<strong class="item-label"><?php esc_html_e( 'Trip Cost:', 'travel-muni' ); ?></strong>
				<span class="value">
				<?php
			...
			<div class="detail-item">
				<strong class="item-label"><?php esc_html_e( 'Trip start date:', 'travel-muni' ); ?></strong>
				<span
		...
		?>
		<div class="detail-item" style="text-align:center;justify-content:center;">
			<strong style="font-size:18px;font-weight:normal;"><?php esc_html_e( 'Payment Details', 'travel-muni' ); ?></strong>
		</div>
		<?php
		$paid_amount = 0;
		if ( is_array( $booking->payments ) && count( $booking->payments ) ) {
			foreach ( $booking->payments as $index => $pid ) {
				...
				?>
				<div class="detail-item">
					<strong class="item-label"><?php echo sprintf( 'Partial Payment #%d', $index + 1 ); ?></strong>
					<span class="value"><?php echo wp_kses( wte_get_formated_price( $payment_amount, '', '', false, false, true ) . " <code>[{$payment_obj->payment_status}]</code>", array( 'code' => array() ) ); ?></span>
				</div>
				<?php
			}
		}
		...
		?>
		<div class="detail-item">
			<strong class="item-label"><?php esc_html_e( 'Payment amount:', 'travel-muni' ); ?></strong>
			<span class="value">
		...
		<div class="detail-item">
			<strong class="item-label"><?php esc_html_e( 'Due amount:', 'travel-muni' ); ?></strong>
			<span class="value">
				<?php echo esc_html( wte_get_formated_price( +$cart_info['total'] - $paid_amount, $currency, '', ! 0 ) ); ?>
			</span>
		</div>
		<div class="detail-item">
			<strong class="item-label"><?php esc_html_e( 'Remarks: ', 'travel-muni' ); ?></strong>
			<div class="value"><?php echo esc_html( WTE_Booking_Response::responses( $payment->payment_status ) ); ?></div>
		</div>
		<div class="detail-item">
			<strong class="item-label"><?php esc_html_e( 'Subtotal:', 'travel-muni' ); ?></strong>
			<span class="value">
				<?php
		...
		?>
		<div class="detail-item">
			<strong class="item-label"><?php esc_html_e( 'Discount (-):', 'travel-muni' ); ?></strong>
			<span class="value">
				<?php
		...
		<div class="detail-item">
			<strong class="item-label"><?php esc_html_e( 'Total:', 'travel-muni' ); ?></strong>
			<span class="value">
				<?php
...
if ( count( $booking->payments ) > 1 ) :
	...
	<div class="thank-you-container-2">
		<div class="wpte-lrf-btn-wrap">
			<a target="_blank" class="wpte-lrf-btn" href="<?php echo esc_url( get_post_type_archive_link( 'trip' ) ); ?>"><?php esc_html_e( 'Book More Trips', 'travel-muni' ); ?></a>
		</div>
		<?php
		...
		if ( ! empty( $user_account_page_id ) ) {
			?>
			<div class="wpte-lrf-btn-wrap">
				<a class="wpte-lrf-btn" href="<?php echo esc_url( get_permalink( $user_account_page_id ) ); ?>"><?php esc_html_e( 'Back to User Dashboard', 'travel-muni' ); ?></a>
			</div>
			<?php
		}
		?>
