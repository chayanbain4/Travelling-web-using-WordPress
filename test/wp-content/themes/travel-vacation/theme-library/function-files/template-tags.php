...
if ( ! function_exists( 'travel_vacation_posted_on_single' ) ) :
    ...
    function travel_vacation_posted_on_single() {
        ...
        $travel_vacation_posted_on = '<span class="post-date"><a href="' . esc_url( get_permalink() ) . '" rel="bookmark"><i class="far fa-clock"></i>' . $time_string . '</a></span>';
		...
		if ( 'post' === get_post_type() && is_singular() ) {
			...
			if ( ! $travel_vacation_hide_tag ) {
				...
				if ( $travel_vacation_tags_list ) {
					...
					printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'travel-vacation' ) . '</span>', $tags_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				}
			}
		}
